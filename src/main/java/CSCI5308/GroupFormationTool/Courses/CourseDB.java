...
public class CourseDB implements ICoursePersistence {
	...
	public List<ICourse> loadAllCourses() throws SQLException {
		...
		try {
			...
			logger.error(CourseDB.class.toString(),
					String.format("action=getCourses status=failure Exception=" + e.getMessage()));
			throw e;
		} finally {
			if (null != proc) {
				proc.cleanup();
	...
	public void loadCourseByID(long id, ICourse course) throws SQLException {
		...
		try {
			...
			if (null != results) {
				...
				logger.info(CourseDB.class.toString(),
						String.format("courseID=%d action=getCourseByID status=success", id));
			}
		} catch (SQLException e) {
			logger.error(CourseDB.class.toString(), String
					.format("courseID=%d action=getCourseByID status=failure exception e=%s", id, e.getMessage()));
			throw e;
		} finally {
			if (null != proc) {
				proc.cleanup();
	...
	public void createCourse(ICourse course) throws SQLException {
		...
		try {
			...
			proc.execute();
			logger.info(CourseDB.class.toString(),
					String.format("course=%s action=createCourse status=success", course.getTitle()));
		} catch (SQLException e) {
			logger.error(CourseDB.class.toString(), String.format(
					"course=%s action=createCourse status=failure exception e=%s", course.getTitle(), e.getMessage()));
			throw e;
		} finally {
			if (null != proc) {
				proc.cleanup();
	...
	public boolean deleteCourse(long id) throws SQLException {
		...
		try {
			...
			logger.error(CourseDB.class.toString(),
					String.format("courseID=%d action=deleteCourse status=failure exception e=%s", id, e.getMessage()));
			throw e;
		} finally {
			if (null != proc) {
				proc.cleanup();
