...
public class PasswordValidatorDB implements IPasswordValidatorPersistence {
	...
	public HashMap<Long, String> loadActivePasswordValidators() throws SQLException {
		...
		try {
			...
			logger.error(PasswordValidatorDB.class.toString(), String
	...
	public String loadConstraintByValidatorId(long id) throws SQLException {
		...
		try {
			...
			logger.error(PasswordValidatorDB.class.toString(), String.format(
					"action=loadConstraintByValidatorId" + " status=failed " + "exception=%s" + e.getMessage()));
			throw e;
		} finally {
			if (null != proc) {
				proc.cleanup();
	...
	public List<String> fetchPreviousPasswordsByBannerID(String bannerID, int constraint) throws SQLException {
		...
		try {
			...
			logger.error(PasswordValidatorDB.class.toString(),
					String.format(
							"bannedID=%s action=fetchPreviousPasswordsByBannerID " + "status=failed" + " exception=%s",
							bannerID, e.getMessage()));
			throw e;
		} finally {
			if (null != proc) {
				proc.cleanup();
