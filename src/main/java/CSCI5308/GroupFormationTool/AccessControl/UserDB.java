...
public class UserDB implements IUserPersistence {
	...
	public void loadUserByID(long id, IUser user) throws SQLException {
		...
		try {
			...
			logger.error(UserDB.class.toString(),
					String.format("userID=%d action=getUserByID status=failure exception e=%s", id, e.getMessage()));
			throw e;
		} finally {
			if (null != proc) {
				proc.cleanup();
	...
	public void loadUserByBannerID(String bannerID, IUser user) throws SQLException {
		...
		try {
			...
			logger.error(UserDB.class.toString(), String.format(
					"userID=%s action=getUserByBannerID status=failure exception e=%s", bannerID, e.getMessage()));
			throw e;
		} finally {
			if (null != proc) {
				proc.cleanup();
	...
	public void createUser(IUser user) throws SQLException {
		...
		try {
			...
			logger.info(UserDB.class.toString(),
					String.format("user=%s action=createUser status=success", user.getBannerID()));
		} catch (SQLException e) {
			logger.error(UserDB.class.toString(), String.format(
					"user=%s action=createUser status=failure exception e=%s", user.getBannerID(), e.getMessage()));
			throw e;
		} finally {
			if (null != proc) {
				proc.cleanup();
