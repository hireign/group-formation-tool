...
public class CustomAuthenticationManager implements AuthenticationManager
{
	...
	private Authentication checkAdmin(String password, User u, Authentication authentication) throws AuthenticationException
	{
		...
		if (password.equals(u.getPassword()))
		{
			...
			token = new UsernamePasswordAuthenticationToken(authentication.getPrincipal(),
																			...
																			rights);
			logger.info(CustomAuthenticationManager.class.toString(),String.format("user=%s action=login status=success", u.getBannerID()));
			return token;
		}
		else
		{
			logger.error(CustomAuthenticationManager.class.toString(),String.format("user=%s action=login status=failure reason=Incorrect password", u.getBannerID()));
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException
	{
		...
		if (u.isInvalidUser())
		{
			logger.info(CustomAuthenticationManager.class.toString(),String.format("user=%s action=login status=failure reason=Account not created", bannerID));
